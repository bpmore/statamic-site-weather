...
- [x] **5.5 Build and verify the Structure band — Constellation.** Its loop had finished (`ALL TASKS COMPLETE`, `v1.0.0`), so built here on `feature/site-weather-band` in `../statamic-constellation` (`6ba64e7`). No lock file, CI resolves fresh with pint and PHPStan level 4 → the A11y Report shape: framework-free `StructureWeather` (analysed, unit-tested), thin adapter excluded from PHPStan with the reason, feature tests that skip without Site Weather. 449 green with it, 443 + 7 skips without. Dev site: migrated, map built, tile reads **Structure: Storm, 10 of 11 pages are orphans** → `/cp/utilities/constellation`.
- [x] **5.6 Build and verify the Content model band — Fallow (née Drift).** Fallow reports content-model drift, not translations, and stores nothing. Built on `feature/site-weather-band` in `../statamic-fallow` (`6838f9f`, `8e24e9c`): a `fallow.report_path` config, a band that reads the audit a site saves there and says what to run until it exists, `--json=path` now creating its directory. 111 green with Site Weather, 104 + 7 skips without. Dev site: audit saved, tile reads **Content model: Storm** → `/cp/utilities/fallow`. README, LISTING and SPEC §9 updated for the renamed band.
- [x] **5.7 Retire the demo provider.** Every band is real; "Translations" has no product. `SiteWeatherDemoProvider` deleted from the dev site and unregistered. The tile now shows only what is measured.

## Follow-ups outside this repo — not tasks for this loop
- All six bands are real: Accessibility (A11y Report), Documents (A11y Docs), Freshness (Lifecycle), Readability (Plain), Structure (Constellation), Content model (Fallow). Wayfinding, Delegated and LMS are not bands in the spec.
- **`../ally-report` is checked out on `readability-core`**, a branch without the band, so Accessibility is absent from the dev-site tile until that checkout carries `feature/site-weather-band` (checkout or merge — a human's call). The tile's behaviour there is the designed one: missing contributor, missing band.
...
**2026-09-15 — Phases 5.5–5.7 (Constellation, Fallow, demo retired).**
- Neither repo was under `~/Herd` — both were private GitHub repos not yet cloned; found with `gh repo list`, cloned to where the build kit expects them. The kit's `products.tsv` still calls Fallow "drift".
- **Fallow changed the contract's assumptions.** SPEC §2 says "if a band needs data nobody is collecting, the band doesn't exist." Fallow collects nothing, so the choice was no band, or a band read from something the site chooses to save. The second is honest as long as the band never computes and says what to run until the artefact exists — and it is opt-in by construction. Rule 1 stands; this is a case for it, not an exception. Fallow itself still never writes; the site's scheduled command does.
- Constellation's band scales by share, not count: ten orphans on forty pages is a storm, ten on forty thousand is fair.
- Three different CI/lock regimes across the six products produced three integration shapes — path-repo `require-dev` (A11y Docs, Lifecycle, Plain), skip-when-absent adapter (A11y Report, Constellation, Fallow), plus a PHPStan exclusion where static analysis runs. All collapse into one plain `require-dev` once Site Weather is on Packagist. **Publishing this package is now the highest-value remaining step.**
- Chrome disconnected again mid-run; verification is the kernel render of `/cp/dashboard` as the CP user, as before.
